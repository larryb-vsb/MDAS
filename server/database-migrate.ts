...
async function checkTablesExist() {
  ...
  const requiredTables = [
    ...
    // MMS Uploader Infrastructure (Schema 2.6.0)
...
// MMS Uploader tables (Schema 2.6.0)
async function createUploaderUploadsTable() {
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS ${sql.identifier(tableName)} (
      ...
      upload_complete TIMESTAMP WITH TIME ZONE,
      uploaded_line_count INTEGER,
      identify_start TIMESTAMP WITH TIME ZONE,
      identify_complete TIMESTAMP WITH TIME ZONE,
      encoding_start TIMESTAMP WITH TIME ZONE,
      encoding_complete TIMESTAMP WITH TIME ZONE,
  ...
  // Add indexes for performance
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_current_phase_idx`)} ON ${sql.identifier(tableName)} (current_phase)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_upload_status_idx`)} ON ${sql.identifier(tableName)} (upload_status)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_start_time_idx`)} ON ${sql.identifier(tableName)} (start_time)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_filename_idx`)} ON ${sql.identifier(tableName)} (filename)`);
}
...
async function createUploaderJsonTable() {
  ...
  // Add indexes for performance
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_upload_id_idx`)} ON ${sql.identifier(tableName)} (upload_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_line_number_idx`)} ON ${sql.identifier(tableName)} (line_number)`);
}
...
async function createUploaderTddfJsonbRecordsTable() {
  ...
  // Add indexes for performance
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_upload_id_idx`)} ON ${sql.identifier(tableName)} (upload_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_record_type_idx`)} ON ${sql.identifier(tableName)} (record_type)`);
}
...
async function createUploaderMastercardDiEditRecordsTable() {
  ...
  // Add indexes for performance
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_upload_id_idx`)} ON ${sql.identifier(tableName)} (upload_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS ${sql.identifier(`${tableName}_processing_status_idx`)} ON ${sql.identifier(tableName)} (processing_status)`);
}