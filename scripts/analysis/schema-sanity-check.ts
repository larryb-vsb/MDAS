 ...
 * 
 * This file maps dev column names to production column names.
 * Auto-generated by: npx tsx scripts/analysis/schema-sanity-check.ts
 * 
 * NOTE: Auto-detection only catches clear renames. Many differences are
 * type mismatches or extra columns, not renames. Review the sanity report
 * and manually add mappings for actual column renames.
 * 
...
export const schemaCompatMap = {\n`;
  ...
  ts += `export type ColumnMapping<T extends TableName> = typeof schemaCompatMap[T];\n\n`;
  ...
  ts += `  return devColumn;\n`;
  ts += `}\n\n`;
  ts += `export function mapIndexDefinition(\n`;
  ts += `  indexDef: string, \n`;
  ts += `  devTable: string, \n`;
  ts += `  prodTable: string\n`;
  ts += `): string {\n`;
  ts += `  let mapped = indexDef.replace(new RegExp(devTable, 'g'), prodTable);\n`;
  ts += `  \n`;
  ts += `  const tableMap = schemaCompatMap[prodTable as TableName];\n`;
  ts += `  if (tableMap) {\n`;
  ts += `    for (const [devCol, prodCol] of Object.entries(tableMap)) {\n`;
  ts += `      mapped = mapped.replace(new RegExp(\`\\\\b\${devCol}\\\\b\`, 'g'), prodCol);\n`;
  ts += `    }\n`;
  ts += `  }\n`;
  ts += `  \n`;
  ts += `  return mapped;\n`;
  ts += `}\n\n`;
  ts += `export function getKnownDifferences(): string {\n`;
  ts += `  return \`\n`;
  ts += `Known Column Mappings (Dev → Prod):\n`;
  ts += `===================================\n`;
  ts += `\${Object.entries(schemaCompatMap)\n`;
  ts += `  .map(([table, cols]) => \n`;
  ts += `    \`\${table}:\\n\${Object.entries(cols).map(([dev, prod]) => \`  - \${dev} → \${prod}\`).join('\\n')}\`\n`;
  ts += `  )\n`;
  ts += `  .join('\\n\\n')}\n`;
  ts += `\n`;
  ts += `Note: These mappings are used by sync scripts to translate \n`;
  ts += `dev column names to production column names.\n`;
  ts += `\`;\n`;
  ts += `}\n`;
...
async function main() {
  try {
    ...
    if (fs.existsSync(compatPath)) {
      console.log(`📄 Compatibility map exists: ${compatPath} (preserved - edit manually)`);
      console.log(`   To regenerate, delete the file and re-run this script.`);
    } else {
      const compatMap = generateCompatMap(report);
      fs.writeFileSync(compatPath, compatMap);
      console.log(`📄 Compatibility map created: ${compatPath}`);
    }
