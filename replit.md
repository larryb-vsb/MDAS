...
**CRITICAL TDDF1 FIELD EXTRACTION FIX (2025-08-09)**: Fixed major field extraction error in TDDF1 merchant calculations. System was correctly using field positions (BH: 69-83, DT: 93-103) but incorrectly aggregating ALL files instead of date-specific files. Updated calculation to process only specific date files, verified against PowerShell validation tool with exact match: $893,361.78 BH net deposits, $897,358.84 DT authorization for July 31st. Merchant totals now show realistic business values instead of inflated amounts.
