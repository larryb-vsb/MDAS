...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Enhanced merchant search endpoint with comprehensive filtering
  app.get('/api/tddf-json/search-merchant', isAuthenticated, async (req, res) => {
    try {
      const { account, recordType, filename, limit = 100 } = req.query;
      
      if (!account || typeof account !== 'string') {
        return res.status(400).json({ error: 'Merchant account number is required' });
      }
      
      const cleanAccount = account.trim();
      if (cleanAccount.length === 0) {
        return res.status(400).json({ error: 'Merchant account number cannot be empty' });
      }
      
      console.log(`[MERCHANT-SEARCH] Searching for merchant account: "${cleanAccount}"`);
      console.log(`[MERCHANT-SEARCH] Filters - Record Type: ${recordType || 'All'}, Filename: ${filename || 'All'}, Limit: ${limit}`);
      
      const tableName = getEnvironmentTableName('uploader_tddf_jsonb_records');
      console.log(`[MERCHANT-SEARCH] Using table: ${tableName}`);
      
      // Build dynamic query with filters
      let whereConditions = ['merchant_account_number = $1'];
      let queryParams = [cleanAccount];
      let paramCount = 1;
      ...
      if (filename && filename !== 'all') {
        paramCount++;
        whereConditions.push(`filename ILIKE $${paramCount}`);
      ...
      const searchQuery = `
        ...
        FROM ${tableName}
        WHERE ${whereConditions.join(' AND ')}
        ORDER BY tddf_processing_datetime DESC NULLS LAST, filename DESC, line_number ASC
        LIMIT $${paramCount + 1}
      `;
      
      queryParams.push(parseInt(limit.toString()) || 100);
      
      const results = await db.execute(sql.raw(searchQuery, queryParams));
      console.log(`[MERCHANT-SEARCH] Found ${results.length} records for merchant: ${cleanAccount}`);
      
      if (results.length === 0) {
        return res.json({
          success: true,
          merchantAccount: cleanAccount,
          totalRecords: 0,
          records: [],
          recordTypeSummary: {},
          filenameSummary: {},
          message: `No records found for merchant account: ${cleanAccount} with applied filters`
        });
      }
      ...
      const processedRecords = results.map(record => {
        let extractedFields = {};
        try {
          extractedFields = typeof record.record_data === 'string' 
            ? JSON.parse(record.record_data) 
            : record.record_data || {};
        } catch (e) {
          console.warn(`[MERCHANT-SEARCH] Failed to parse record_data for record ${record.id}`);
        }
        ...
        return {
          ...
          merchantAccount: record.merchant_account_number,
          extractedFields,
          createdAt: record.created_at,
          tddfProcessingDatetime: record.tddf_processing_datetime,
          parsedDatetime: record.parsed_datetime,
          recordIdentifier: record.record_identifier,
          fieldCount: record.field_count
        };
      });
      
      // Group by record type and filename for summaries
      ...
      processedRecords.forEach(record => {
        ...
        recordTypeSummary[type] = (recordTypeSummary[type] || 0) + 1;
        filenameSummary[filename] = (filenameSummary[filename] || 0) + 1;
      });
      ...
      console.error('[MERCHANT-SEARCH] Error searching merchant records:', error);
      res.status(500).json({ 
        error: 'Failed to search merchant records',
        details: error.message 
      });
    }
  });

  // Get unique merchant accounts for autocomplete
  app.get('/api/tddf-json/merchant-accounts', isAuthenticated, async (req, res) => {
    try {
      const { limit = 50, search = '' } = req.query;
      const tableName = getEnvironmentTableName('uploader_tddf_jsonb_records');
      
      console.log(`[MERCHANT-ACCOUNTS] Fetching merchant accounts with search: "${search}"`);
      
      let searchCondition = '';
      let queryParams = [];
      
      if (search && search.toString().trim()) {
        searchCondition = 'WHERE merchant_account_number ILIKE $1';
      ...
      const accountsQuery = `
        ...
        AND TRIM(merchant_account_number) != ''
        GROUP BY merchant_account_number
        ORDER BY record_count DESC, merchant_account_number ASC
        LIMIT $${queryParams.length + 1}
      `;
      
      queryParams.push(parseInt(limit.toString()) || 50);
      
      const results = await db.execute(sql.raw(accountsQuery, queryParams));
      
      const merchantAccounts = results.map(row => ({
        accountNumber: row.merchant_account_number,
        recordCount: parseInt(row.record_count) || 0,
        recordTypes: parseInt(row.record_types) || 0,
        fileCount: parseInt(row.file_count) || 0,
        firstSeen: row.first_seen,
        lastSeen: row.last_seen
      }));
      
      console.log(`[MERCHANT-ACCOUNTS] Found ${merchantAccounts.length} unique merchant accounts`);
      ...
      console.error('[MERCHANT-ACCOUNTS] Error fetching merchant accounts:', error);
      res.status(500).json({ 
