...
- **Production Schema Management**: Version-controlled database schema system with timestamped SQL files (Version 2.9.0, last updated 2025-11-17). Auto-generated from development database via `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`. Creates both `production-schema-YYYY-MM-DD_HH-MM-SS.sql` (timestamped in filename for easy tracking) and `production-schema.sql` (current version reference). Includes 70 tables with PostgreSQL COMMENT metadata for documentation, 420+ performance indexes with proper production naming (no dev_ prefixes). Safe to run on empty OR existing databases using IF NOT EXISTS - creates missing tables/indexes, skips existing ones, preserves all data. File size: ~117 KB of production-ready SQL. Regenerate after schema changes with: `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`. Cleanup script available at `scripts/cleanup-bad-indexes.sql` for removing any legacy dev_-prefixed indexes from production.
