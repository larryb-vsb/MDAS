...
- **TDDF1 Table View Tab**: Added comprehensive table view tab to TDDF1 dashboard providing multi-day daily breakdown. Shows last 30 days of data with date, record counts (BH, DT), total records, file count, net deposits, and authorization values. Table features clickable rows for date navigation, highlighting of selected date, monospaced fonts for numbers, hover effects, and data source indicator (cache vs. master table). Backend endpoint `/api/tddf1/multi-day-breakdown` queries cache first (fast) with automatic fallback to master table (complete). Supports configurable date ranges and provides accurate transaction data across multiple days.
