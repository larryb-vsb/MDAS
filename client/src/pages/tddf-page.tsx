      ...
      <CardContent>
        {bhLoading ? (
          ...
          <div className="space-y-4">
            {/* Table Header - BH specific columns (only 4 required fields) */}
            ...
            {/* Table Rows - BH specific data display (only 4 required fields) */}
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div><span className="font-medium">Transaction Code (52-55):</span> {detailsRecord.transactionCode || 'N/A'}</div>
                      <div><span className="font-medium">Batch Julian Date (64-68):</span> {detailsRecord.batchJulianDate || 'N/A'}</div>
...
// G2 Records Table Component
function G2RecordsTable() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["/api/tddf/other-records", "G2", currentPage, itemsPerPage],
    queryFn: async () => {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch G2 records");
      }
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>
            G2 Records ({totalRecords}) - Merchant General Data 2 Extension
          </CardTitle>
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center py-8">
            <div className="text-muted-foreground">Loading G2 records...</div>
          </div>
        ) : records.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            No G2 records found
        ...
        {/* G2 Record Details Dialog */}
        <Dialog open={!!detailsRecord} onOpenChange={(open) => !open && setDetailsRecord(null)}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>G2 Record Details - ID: {detailsRecord?.id}</DialogTitle>
            </DialogHeader>
            {detailsRecord && (
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <h4 className="font-medium mb-2">G2 Record Identifier</h4>
                    <div className="space-y-2 text-sm">
                  ...
                  <div>
                    <h4 className="font-medium mb-2">G2 Extension Data</h4>
                    <div className="space-y-2 text-sm">
                ...
                {detailsRecord.recordData && (
                  <div>
                    <h4 className="font-medium mb-2">Complete G2 Field Data</h4>
                    <div className="bg-gray-50 p-3 rounded text-xs font-mono overflow-x-auto max-h-48">
                      {JSON.stringify(detailsRecord.recordData, null, 2)}
                    </div>
                  </div>
                )}
              </div>
...
// AD Records Table Component
function ADRecordsTable() {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(20);
  const [selectedRecords, setSelectedRecords] = useState<Set<number>>(new Set());
  const [detailsRecord, setDetailsRecord] = useState<any>(null);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["/api/tddf/other-records", "AD", currentPage, itemsPerPage],
    queryFn: async () => {
      const params = new URLSearchParams({
        recordType: "AD",
        page: currentPage.toString(),
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch AD records");
      }
      return response.json();
    },
  });
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>
            AD Records ({totalRecords}) - Merchant Adjustment Extension
          </CardTitle>
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">Show:</span>
            <Select
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center py-8">
            <div className="text-muted-foreground">Loading AD records...</div>
          </div>
        ) : records.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            No AD records found
        ...
        <Dialog open={!!detailsRecord} onOpenChange={() => setDetailsRecord(null)}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>AD Record Details - Merchant Adjustment Extension</DialogTitle>
            </DialogHeader>
            {detailsRecord && (
              <div className="space-y-6">
                <div>
...
function E1RecordsTable() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["/api/tddf/other-records", "E1", currentPage, itemsPerPage],
    queryFn: async () => {
      const params = new URLSearchParams({
        recordType: "E1",
        page: currentPage.toString(),
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch E1 records");
      }
      return response.json();
    },
  });
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>
            E1 Records ({totalRecords}) - Merchant General Data 2 Extension
          </CardTitle>
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">Show:</span>
            <Select
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center py-8">
            <div className="text-muted-foreground">Loading E1 records...</div>
          </div>
        ) : records.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            No E1 records found
        ...
        {/* E1 Record Details Dialog */}
        <Dialog open={!!detailsRecord} onOpenChange={(open) => !open && setDetailsRecord(null)}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>E1 Record Details - ID: {detailsRecord?.id}</DialogTitle>
            </DialogHeader>
            {detailsRecord && (
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <h4 className="font-medium mb-2">E1 Record Identifier</h4>
                    <div className="space-y-2 text-sm">
                      <div><span className="font-medium">Record Type:</span> {detailsRecord.recordType}</div>
                      <div><span className="font-medium">Record ID:</span> {detailsRecord.id}</div>
                      <div><span className="font-medium">Reference Number:</span> {detailsRecord.referenceNumber || 'N/A'}</div>
                ...
                {detailsRecord.recordData && (
                  <div>
                    <h4 className="font-medium mb-2">Complete EMV Field Data</h4>
                    <div className="bg-gray-50 p-3 rounded text-xs font-mono overflow-x-auto max-h-48">
                      {JSON.stringify(detailsRecord.recordData, null, 2)}
                    </div>
                  </div>
