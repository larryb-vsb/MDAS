...
- **Terminal Management**: Efficient UPSERT logic for terminal imports with unique constraints and automatic timestamp updates.
- **Analytics**: Comprehensive daily merchant breakdown dashboard with full record type details, optimized with multiple performance indexes. Includes a compact monthly calendar heat map for terminal activity. Standardized `mcc` property usage across backend and frontend for accurate reporting.
