...
- **Terminal CSV Import UPSERT & Detection Fix (Oct 31, 2025)**: Fixed terminal file type detection to prevent CSV files from being misidentified as TDDF by adding comma check in detectTddfPattern() function. Implemented efficient UPSERT logic (INSERT ... ON CONFLICT DO UPDATE) for terminal imports replacing SELECT/UPDATE/INSERT pattern. Added UNIQUE constraint on v_number column to prevent duplicates. Terminal imports now automatically update existing terminals or create new ones in single atomic operation, with last_update timestamp populated for both operations. Update source tracking shows filename in update_source field. This enables re-importing terminal CSV files to refresh data without creating duplicates.
