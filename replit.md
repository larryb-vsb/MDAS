...
- **Production Schema Management**: Version-controlled database schema system with `production-schema.sql` (Version 2.8.0, last updated 2025-11-17) that creates complete production database from scratch. Auto-generated from development database via `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`. Includes 70 tables (merchants, TDDF records, partitioned TDDF JSONB quarterly tables, cache configuration, pre-cache tables, system tables, uploader tables) with 420 performance indexes. Safe to run multiple times using IF NOT EXISTS. File size: 111.4 KB of production-ready SQL. Regenerate after schema changes with: `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`.
