...
import { Download, List, RefreshCw, CheckCircle, Trash2 } from 'lucide-react';
...
export default function BackupHistoryDialog({ onClose }: BackupHistoryProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/settings/backup/${backupToDelete}`, {
        method: 'DELETE'
      });
      ...
      // Invalidate the query to refresh backup history
      queryClient.invalidateQueries({ queryKey: ["/api/settings/backup/history"] });
      
      toast({
        title: "Backup moved to trash",
        description: "The backup file has been moved to trash successfully."
      });
    } catch (error) {
      ...
      <AlertDialog open={!!backupToDelete} onOpenChange={(open) => !open && setBackupToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This backup will be moved to trash. It won't appear in the backup history,
              but the file will remain on the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
