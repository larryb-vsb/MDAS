  ...
  try {
    ...
    for (const file of julyFiles) {
      try {
        ...
        // Create the production TDDF1 file table using raw SQL
        const createTableQuery = `
          CREATE TABLE IF NOT EXISTS ${productionTableName} (
        ...
        for (const line of lines) {
          ...
          // Insert record into table using parameterized query
          const insertQuery = `
            INSERT INTO ${productionTableName} (
              raw_line, record_type, processing_date, transaction_amount, bh_net_deposit
            ) VALUES ($1, $2, $3, $4, $5)
          `;
          
          await sql.unsafe(insertQuery, [line, recordType, processingDate, transactionAmount, bhNetDeposit]);
          recordCount++;
        }
    ...
    console.log(`📊 Remaining files: ${1131 - processedCount - errorCount}`);
