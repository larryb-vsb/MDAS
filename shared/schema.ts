...
// File-based TDDF table name helper
export function getFileBasedTddfTableName(fileName: string): string {
  const NODE_ENV = typeof process !== 'undefined' ? process.env.NODE_ENV : 'production';
  const prefix = NODE_ENV === 'development' ? 'dev_' : '';
  ...
  const cleanName = fileName
    ...
    .toLowerCase()
    .substring(0, 50); // Limit length for PostgreSQL
  
  return `${prefix}tddf1_${cleanName}`;
}

// Dynamic file-based TDDF table schema function
export function createFileBasedTddfTable(tableName: string) {
  return pgTable(tableName, {
    id: serial("id").primaryKey(),
    
    // Universal timestamp for precise chronological ordering
    universalTimestamp: numeric("universal_timestamp", { precision: 20, scale: 3 }), // Milliseconds since epoch with 100ms line offset
    ...
    recordIdentifier: text("record_identifier"), // Positions 18-19: Record type (DT, BH, P1, etc.)
    bankNumber: text("bank_number"), // Positions 20-23: Global Payments bank number
    
    // Record type information
    recordType: text("record_type").notNull(), // DT, BH, P1, P2, E1, G2, AD, DR, CK, LG, GE, etc.
    
    // Account and merchant fields (common across record types)
    ...
    // Core transaction fields (for DT records)
    referenceNumber: text("reference_number"), // Reference number
    transactionDate: date("transaction_date"), // Transaction date
    transactionAmount: numeric("transaction_amount", { precision: 15, scale: 2 }), // Transaction amount
    batchJulianDate: text("batch_julian_date"), // Batch Julian date
    netDeposit: numeric("net_deposit", { precision: 17, scale: 2 }), // Net deposit amount
    cardholderAccountNumber: text("cardholder_account_number"), // Cardholder account
    
    // Batch header fields (for BH records)
    batchDate: text("batch_date"), // Batch date
    batchId: text("batch_id"), // Batch ID
    
    // Purchasing card fields (for P1, P2 records)
    purchaseIdentifier: text("purchase_identifier"), // Purchase identifier
    customerCode: text("customer_code"), // Customer code
    merchantName: text("merchant_name"), // Merchant name
    merchantAddress: text("merchant_address"), // Merchant address
    merchantCity: text("merchant_city"), // Merchant city
    merchantState: text("merchant_state"), // Merchant state
    merchantZip: text("merchant_zip"), // Merchant ZIP
    
    // Flexible data storage for all record types
    recordData: jsonb("record_data"), // Complete structured data for the record type
    
    // System and audit fields
    sourceFileId: text("source_file_id"), // Reference to uploaded file
    sourceRowNumber: integer("source_row_number"), // Row number in source file
    recordedAt: timestamp("recorded_at").defaultNow().notNull(), // When record was processed
    rawData: jsonb("raw_data"), // Complete raw line data
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull()
  }, (table) => ({
    universalTimestampIndex: index(`${tableName}_universal_timestamp_idx`).on(table.universalTimestamp),
    recordTypeIndex: index(`${tableName}_record_type_idx`).on(table.recordType),
    merchantAccountIndex: index(`${tableName}_merchant_account_idx`).on(table.merchantAccountNumber),
    transactionDateIndex: index(`${tableName}_transaction_date_idx`).on(table.transactionDate),
    sequenceNumberIndex: index(`${tableName}_sequence_number_idx`).on(table.sequenceNumber)
  }));
}
