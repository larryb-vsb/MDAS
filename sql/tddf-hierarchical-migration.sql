-- TDDF Hierarchical Architecture Migration Script
-- Schema Version: 2.4.0
-- Date: July 23, 2025
-- Purpose: Create separate tables for each TDDF record type with hierarchical relationships

-- ============================================================================
-- DEVELOPMENT ENVIRONMENT TABLES (dev_ prefix)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS dev_tddf_transaction_records (
    id SERIAL PRIMARY KEY,
    
    -- Link to batch header
    batch_header_id INTEGER REFERENCES dev_tddf_batch_headers(id),
...
CREATE INDEX IF NOT EXISTS tddf_dt_reference_number_idx ON dev_tddf_transaction_records(reference_number);
CREATE INDEX IF NOT EXISTS tddf_dt_merchant_account_idx ON dev_tddf_transaction_records(merchant_account_number);
CREATE INDEX IF NOT EXISTS tddf_dt_transaction_date_idx ON dev_tddf_transaction_records(transaction_date);
CREATE INDEX IF NOT EXISTS tddf_dt_merchant_name_idx ON dev_tddf_transaction_records(merchant_name);
CREATE INDEX IF NOT EXISTS tddf_dt_batch_header_idx ON dev_tddf_transaction_records(batch_header_id);
...
CREATE TABLE IF NOT EXISTS dev_tddf_purchasing_extensions (
    ...
    transaction_record_id INTEGER REFERENCES dev_tddf_transaction_records(id),
...
CREATE INDEX IF NOT EXISTS tddf_pe_transaction_record_idx ON dev_tddf_purchasing_extensions(transaction_record_id);
...
CREATE INDEX IF NOT EXISTS tddf_or_record_type_idx ON dev_tddf_other_records(record_type);
CREATE INDEX IF NOT EXISTS tddf_or_merchant_account_idx ON dev_tddf_other_records(merchant_account_number);
CREATE INDEX IF NOT EXISTS tddf_or_transaction_record_idx ON dev_tddf_other_records(transaction_record_id);

-- ============================================================================
-- PRODUCTION ENVIRONMENT TABLES (no prefix)
-- ============================================================================
...
-- Verify development tables were created
SELECT 
    'Development Tables Created' as status,
    COUNT(*) as table_count
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN (
    'dev_tddf_batch_headers',
    'dev_tddf_transaction_records', 
...
SELECT 
    'Production Tables Created' as status,
    COUNT(*) as table_count
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN (
...
SELECT 
    ...
    indexname
FROM pg_indexes 
WHERE tablename LIKE '%tddf_%' 
AND (tablename LIKE 'dev_%' OR tablename IN ('tddf_batch_headers', 'tddf_transaction_records', 'tddf_purchasing_extensions', 'tddf_other_records'))
