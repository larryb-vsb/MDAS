...
- **MCC/TDDF Transactions Tab**: Standalone Transactions page (`/transactions`) with dual-tab interface: ACH Transactions (existing ACH data with sorting, pagination, search) and MCC/TDDF tab displaying last 100 DT records from TDDF processing system. MCC/TDDF tab features formatted transaction display with card type badges, merchant account/name lookup, transaction dates, amounts, and file metadata. Uses `/api/tddf-records/dt-latest` endpoint with environment-aware table selection and merchant name resolution via `/api/merchants/lookup-map`.
