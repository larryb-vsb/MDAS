...
// TDDF1 Dashboard Database Tables

// TDDF1 Widget Configurations Table
export const tddf1WidgetConfigs = pgTable(getTableName("tddf1_widget_configs"), {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull().references(() => users.id),
  widget_id: text("widget_id").notNull(), // 'stats', 'recent', 'files', 'database'
  ...
  userWidgetIdx: index("tddf1_widget_configs_user_widget_idx").on(table.user_id, table.widget_id),
  positionIdx: index("tddf1_widget_configs_position_idx").on(table.position),
  visibleIdx: index("tddf1_widget_configs_visible_idx").on(table.visible)
}));

// TDDF1 Dashboard Statistics Cache
export const tddf1DashboardCache = pgTable(getTableName("tddf1_dashboard_cache"), {
  id: serial("id").primaryKey(),
  cache_key: text("cache_key").notNull().unique(), // 'transaction_stats', 'recent_activity', 'file_management', 'database_status'
  widget_type: text("widget_type").notNull(), // 'stats', 'recent', 'files', 'database'
  cache_data: jsonb("cache_data").notNull(), // Pre-computed widget data
  record_count: integer("record_count").notNull().default(0),
  processing_time_ms: integer("processing_time_ms").notNull().default(0),
  last_update_datetime: timestamp("last_update_datetime").defaultNow().notNull(),
  expires_at: timestamp("expires_at").notNull(),
  metadata: jsonb("metadata"), // Widget-specific metadata
  created_at: timestamp("created_at").defaultNow().notNull(),
  created_by: text("created_by"),
  data_sources: jsonb("data_sources"), // Tables and sources used for this widget
}, (table) => ({
  widgetTypeIdx: index("tddf1_dashboard_cache_widget_type_idx").on(table.widget_type),
  lastUpdateIdx: index("tddf1_dashboard_cache_last_update_idx").on(table.last_update_datetime),
  expiresAtIdx: index("tddf1_dashboard_cache_expires_at_idx").on(table.expires_at)
}));

// TDDF1 User Preferences Table
export const tddf1UserPreferences = pgTable(getTableName("tddf1_user_preferences"), {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull().references(() => users.id),
  layout_mode: text("layout_mode").notNull().default("grid"), // grid, list, compact
  auto_refresh: boolean("auto_refresh").notNull().default(false),
  refresh_interval: integer("refresh_interval").notNull().default(30000), // milliseconds
  theme: text("theme").notNull().default("system"), // light, dark, system
  widget_order: jsonb("widget_order"), // Array of widget IDs in preferred order
  quick_filters: jsonb("quick_filters"), // Saved filter configurations
  date_range_preference: text("date_range_preference").default("last_30_days"),
  show_advanced_metrics: boolean("show_advanced_metrics").notNull().default(false),
  created_at: timestamp("created_at").defaultNow().notNull(),
  updated_at: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
  userIdx: index("tddf1_user_preferences_user_idx").on(table.user_id),
  layoutModeIdx: index("tddf1_user_preferences_layout_mode_idx").on(table.layout_mode),
  autoRefreshIdx: index("tddf1_user_preferences_auto_refresh_idx").on(table.auto_refresh)
}));

// TDDF1 Activity Log Table
export const tddf1ActivityLog = pgTable(getTableName("tddf1_activity_log"), {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").references(() => users.id),
  action_type: text("action_type").notNull(), // 'widget_moved', 'widget_resized', 'filter_applied', 'data_refreshed'
  widget_id: text("widget_id"), // Widget that was acted upon (if applicable)
  action_details: jsonb("action_details"), // Detailed action information
  session_id: text("session_id"), // Browser session identifier
  ip_address: text("ip_address"),
  user_agent: text("user_agent"),
  timestamp: timestamp("timestamp").defaultNow().notNull(),
  metadata: jsonb("metadata"), // Additional context about the action
}, (table) => ({
  userActionIdx: index("tddf1_activity_log_user_action_idx").on(table.user_id, table.action_type),
  timestampIdx: index("tddf1_activity_log_timestamp_idx").on(table.timestamp),
  widgetIdx: index("tddf1_activity_log_widget_idx").on(table.widget_id),
  sessionIdx: index("tddf1_activity_log_session_idx").on(table.session_id)
}));

// TDDF1 Data Sources Configuration
export const tddf1DataSources = pgTable(getTableName("tddf1_data_sources"), {
  id: serial("id").primaryKey(),
  source_name: text("source_name").notNull().unique(), // 'transactions', 'merchants', 'files', 'cache'
  table_name: text("table_name").notNull(), // Actual database table name
  refresh_strategy: text("refresh_strategy").notNull().default("on_demand"), // on_demand, scheduled, real_time
  refresh_interval: integer("refresh_interval").default(300000), // milliseconds (5 minutes default)
  last_refresh: timestamp("last_refresh"),
  next_refresh: timestamp("next_refresh"),
  is_active: boolean("is_active").notNull().default(true),
  connection_config: jsonb("connection_config"), // Database connection parameters
  query_template: text("query_template"), // SQL template for data extraction
  transformation_rules: jsonb("transformation_rules"), // Data transformation configuration
  created_at: timestamp("created_at").defaultNow().notNull(),
  updated_at: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
  sourceNameIdx: index("tddf1_data_sources_source_name_idx").on(table.source_name),
  refreshStrategyIdx: index("tddf1_data_sources_refresh_strategy_idx").on(table.refresh_strategy),
  isActiveIdx: index("tddf1_data_sources_is_active_idx").on(table.is_active),
  nextRefreshIdx: index("tddf1_data_sources_next_refresh_idx").on(table.next_refresh)
}));

// Zod schemas for TDDF1 tables
export const insertTddf1WidgetConfigsSchema = createInsertSchema(tddf1WidgetConfigs).omit({
  id: true,
  created_at: true,
  updated_at: true
});

export const insertTddf1DashboardCacheSchema = createInsertSchema(tddf1DashboardCache).omit({
  id: true,
  created_at: true
});

export const insertTddf1UserPreferencesSchema = createInsertSchema(tddf1UserPreferences).omit({
  id: true,
  created_at: true,
  updated_at: true
});

export const insertTddf1ActivityLogSchema = createInsertSchema(tddf1ActivityLog).omit({
  id: true,
  timestamp: true
});

export const insertTddf1DataSourcesSchema = createInsertSchema(tddf1DataSources).omit({
  id: true,
  created_at: true,
  updated_at: true
});

// Export types for TDDF1 tables
export type Tddf1WidgetConfig = typeof tddf1WidgetConfigs.$inferSelect;
export type InsertTddf1WidgetConfig = z.infer<typeof insertTddf1WidgetConfigsSchema>;

export type Tddf1DashboardCache = typeof tddf1DashboardCache.$inferSelect;
export type InsertTddf1DashboardCache = z.infer<typeof insertTddf1DashboardCacheSchema>;

export type Tddf1UserPreferences = typeof tddf1UserPreferences.$inferSelect;
export type InsertTddf1UserPreferences = z.infer<typeof insertTddf1UserPreferencesSchema>;

export type Tddf1ActivityLog = typeof tddf1ActivityLog.$inferSelect;
export type InsertTddf1ActivityLog = z.infer<typeof insertTddf1ActivityLogSchema>;

export type Tddf1DataSources = typeof tddf1DataSources.$inferSelect;
export type InsertTddf1DataSources = z.infer<typeof insertTddf1DataSourcesSchema>;
