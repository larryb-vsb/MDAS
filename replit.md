...
**MERCHANT ACCOUNT NUMBER FIELD DISPLAY FIXED (2025-08-29)**: Successfully resolved field extraction and display issues for Merchant Account Number in TDDF viewer. Fixed nested JSONB structure parsing where fields were buried inside `record_data.extractedFields.merchantAccountNumber`. Enhanced API endpoint to properly flatten nested field structure and added blue highlighting for Merchant Account Number field visibility. Field now displays correctly at positions 24-39 (16 chars) across all record types (BH, DT, P1) with value "0675900000002881" confirmed working.

**KING SERVER DATABASE CONNECTION & MERCHANT SEARCH IMPLEMENTED (2025-08-29)**: Successfully connected to Neon King server database (ep-shy-king-aasxdlh7) and implemented merchant account number search functionality. Added `merchant_account_number` column to `dev_uploader_tddf_jsonb_records` table with automatic extraction from positions 24-39 of raw TDDF lines. Global merchant search API operational with merchant account `0675900000002881` confirmed working. TDDF encoder updated to automatically populate merchant account numbers for future uploads. Note: Database connection discrepancies exist between Replit SQL tools and Neon interface - user can see schema changes that system cannot detect, but core functionality remains operational.
