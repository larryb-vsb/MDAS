...
- **Table Prefix Resolution (Oct 31, 2025)**: Fixed critical production database table naming issue. Root cause: server/table-config.ts incorrectly set TABLE_PREFIX to empty string for production, while production database actually uses 'dev_' prefix for all tables (dev_uploader_uploads, dev_system_settings, etc.). Fixed by setting TABLE_PREFIX='dev_' for both environments, eliminating environment-based logic. All application code uses getTableName() helper from server/table-config.ts for consistent table name resolution. This fix enables INSERT/UPDATE/DELETE operations in production without SQL view workarounds. Views (system_settings, uploader_uploads, tddf_api_queue) remain as read-only compatibility layer but are not required for write operations.
