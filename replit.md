...
- **Production Schema Management**: Version-controlled database schema system with timestamped SQL files (Version 2.9.0, last updated 2025-11-17). Auto-generated from development database via `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`. Creates both `sql/production-schema-YYYY-MM-DD_HH-MM-SS.sql` (timestamped in filename for easy tracking) and `sql/production-schema.sql` (current version reference) in the `sql/` directory. Includes 70 tables with PostgreSQL COMMENT metadata for documentation, 63 auto-created sequences, 420+ performance indexes with proper production naming (no dev_ prefixes). Safe to run on empty OR existing databases - no transaction wrapper allows each statement to run independently. File size: ~120 KB of production-ready SQL. Regenerate after schema changes with: `NODE_ENV=development npx tsx scripts/simple-schema-dump.ts`. Run against production with: `./scripts/run-production-schema.sh` (logs saved to `logs/` directory). Cleanup script available at `scripts/cleanup-bad-indexes.sql` for removing any legacy dev_-prefixed indexes from production.
